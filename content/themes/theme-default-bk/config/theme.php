<?php

// auth_ui: optional 'v2' for common/auth-v2 UI (default v1 = common/auth). See AuthController::_getAuthUiVersion().

return [
    'path' => 'giao-dien-web',
    'version' => '1.0.0',
    'type' => 'web',
    'parent' => null,
    'auth_ui' => 'v1',
    'theme' => [
        'name' => 'Giao diện Frontend',
        'version' => '1.0.0',
        'description' => 'Giao diện Frontend - Fast & Easy Vibe Code with AI',
        'author' => 'CMS Team',
        'author_url' => 'https://cmsfullform.net',
        'support_url' => 'https://cmsfullform.net/support',
        'documentation_url' => 'https://cmsfullform.net/docs',
        'status' => false, // Default inactive
        'downloads' => 0,
        'rating' => 0,
        'category' => 'Business, Portfolio, Corporate',
        'tags' => ['responsive', 'modern', 'clean', 'business'],
        'screenshot' => 'assets/images/screenshot.png',
        'min_php_version' => '7.4',
        'min_cms_version' => '1.0.0',
        'features' => [
            'Responsive Design',
            'Modern UI/UX',
            'SEO Optimized',
            'Fast Loading',
            'Customizable Colors',
            'Multiple Layouts'
        ],
        'compatibility' => [
            'browsers' => ['Chrome', 'Firefox', 'Safari', 'Edge'],
            'devices' => ['Desktop', 'Tablet', 'Mobile']
        ],
        'changelog' => [
            '1.0.0' => [
                'date' => '2024-01-01',
                'changes' => [
                    'Initial release',
                    'Basic frontend templates',
                    'Backend admin interface',
                    'Responsive design'
                ]
            ]
        ]
    ]
];
