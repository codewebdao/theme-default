...
class jModalManager {
    constructor() {
        ...
        // Cache when body exists (script may run in <head>)
        this.cachedElements = {
            body: document.body,
        ...
        if (this.cachedElements.body) {
            this.scrollbarWidth = this.getScrollbarWidth();
        } else {
            this.scheduleSetup(() => this.lazyInitScrollbarWidth());
        }
        
        this.scheduleSetup(() => this.setupGlobalListeners());
    }
    
    lazyInitScrollbarWidth() {
     ...
     */
    getScrollbarWidth() {
        const body = this.cachedElements.body || document.body;
        if (!body) return 0;
    ...
    updateBodyScroll() {
        const body = this.cachedElements.body || document.body;
        if (!body) return;
        if (!this.cachedElements.body) this.cachedElements.body = body;
