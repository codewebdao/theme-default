 ...
 * Head + Schema: mặc định từ context (BlogPosting). Override: Head::setTitle() / filter render.head.defaults.
 */
...
view_header(['layout' => $layout ?? 'single-blogs']);
?>
<div class="container mx-auto px-4 py-8">
    <?php if ($p) : ?>
        <article class="prose prose-slate max-w-none" itemscope itemtype="https://schema.org/BlogPosting">
            <h1 class="text-2xl font-bold text-slate-800" itemprop="headline"><?php echo h($postTitle); ?></h1>
            <?php if ($datePub || $author) : ?>
                <p class="mt-1 text-sm text-slate-500">
                    <?php if ($datePub) : ?>
                        <time datetime="<?php echo e(date('c', is_numeric($datePub) ? $datePub : strtotime($datePub))); ?>" itemprop="datePublished"><?php echo e(date($dateFmt, is_numeric($datePub) ? $datePub : strtotime($datePub))); ?></time>
                    <?php endif; ?>
                    <?php if ($author) : ?>
                        <span class="ml-2" itemprop="author"><?php echo h($author); ?></span>
                    <?php endif; ?>
                </p>
            <?php endif; ?>
            <?php
