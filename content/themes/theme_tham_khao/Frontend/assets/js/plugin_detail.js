...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize plugin detail functionality
    initializePluginDetail();
});

function initializePluginDetail() {
...
function initializeDownloadTracking() {
    const downloadButtons = document.querySelectorAll('a[href*="/download/"], a[href*="/install/"]');
    
    downloadButtons.forEach(button => {
        button.addEventListener('click', function() {
            const pluginName = this.href.split('/').pop();
            trackDownload(pluginName);
        });
    });
}
...
function saveFavoriteState(button) {
    const pluginSlug = button.dataset.pluginSlug || '';
    const isActive = button.classList.contains('active');
    
    if (!pluginSlug) return;
    
    let favorites = JSON.parse(localStorage.getItem('plugin_favorites') || '[]');
    
    if (isActive) {
        if (!favorites.includes(pluginSlug)) {
            favorites.push(pluginSlug);
        }
    } else {
        favorites = favorites.filter(slug => slug !== pluginSlug);
    }
...
function loadFavoriteStates() {
    const favorites = JSON.parse(localStorage.getItem('plugin_favorites') || '[]');
    const favoriteButtons = document.querySelectorAll('[data-plugin-slug]');
