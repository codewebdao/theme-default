...
<div class="" x-data="librariesManager('<?= $type ?>', <?= htmlspecialchars(json_encode($items), ENT_QUOTES, 'UTF-8') ?>)">
    <div class="flex flex-col gap-4">
        ...
        <?php echo View::include('parts/ui/notification', ['type' => 'info', 'message' => 'This is Development Mode (may be have high risk), for Disable, set development to False at Config.php']); ?>
        ...
        <div dir="ltr" data-orientation="horizontal" class="w-full">
            ...
            <div :data-state="mainTab === 'installed' ? 'active' : 'inactive'" data-orientation="horizontal" role="tabpanel" :aria-labelledby="'tab-installed'" tabindex="0" class="ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 mt-4" :hidden="mainTab !== 'installed'">
                ...
                <!-- Filter: status + (themes) website vs admin — cùng hàng -->
                <div class="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3">
                    <div dir="ltr" data-orientation="horizontal" class="inline-flex flex-1 min-w-0">
                        <div role="tablist" aria-orientation="horizontal" class="inline-flex items-center justify-center rounded-md bg-muted p-1 px-2 text-muted-foreground h-9 w-full sm:w-auto" tabindex="0" data-orientation="horizontal" style="outline: none;">
                            <button type="button" role="tab" :aria-selected="installedFilterTab === 'all'" :data-state="installedFilterTab === 'all' ? 'active' : 'inactive'" @click="installedFilterTab = 'all'" class="inline-flex items-center justify-center whitespace-nowrap rounded-sm px-2.5 py-1 font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm text-xs" tabindex="-1" data-orientation="horizontal" data-radix-collection-item=""><?= __('all') ?> (<span x-text="type === 'themes' ? filteredByTypeLength : items.length"></span>)</button>
                            <button type="button" role="tab" :aria-selected="installedFilterTab === 'active'" :data-state="installedFilterTab === 'active' ? 'active' : 'inactive'" @click="installedFilterTab = 'active'" class="inline-flex items-center justify-center whitespace-nowrap rounded-sm px-2.5 py-1 font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm text-xs" tabindex="-1" data-orientation="horizontal" data-radix-collection-item=""><?= __('active') ?> (<span x-text="activeCountScoped"></span>)</button>
                            <button type="button" role="tab" :aria-selected="installedFilterTab === 'inactive'" :data-state="installedFilterTab === 'inactive' ? 'active' : 'inactive'" @click="installedFilterTab = 'inactive'" class="inline-flex items-center justify-center whitespace-nowrap rounded-sm px-2.5 py-1 font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm text-xs" tabindex="-1" data-orientation="horizontal" data-radix-collection-item=""><?= __('inactive') ?> (<span x-text="inactiveCountScoped"></span>)</button>
                        </div>
                    </div>
                    <template x-if="type === 'themes'">
                        <div dir="ltr" class="inline-flex flex-1 min-w-0 sm:max-w-md">
                            <div role="tablist" class="inline-flex items-center justify-center rounded-md bg-muted p-1 px-2 text-muted-foreground h-9 max-w-[340px]">
                                <button type="button" @click="installedTypeTab = 'all'" :class="installedTypeTab === 'all' ? 'bg-background text-foreground shadow-sm' : ''" class="inline-flex flex-1 sm:flex-initial items-center justify-center whitespace-nowrap rounded-sm px-2.5 py-1 font-medium transition-all text-xs"><?= __('All types') ?> (<span x-text="items.length"></span>)</button>
                                <button type="button" @click="installedTypeTab = 'web'" :class="installedTypeTab === 'web' ? 'bg-background text-foreground shadow-sm' : ''" class="inline-flex flex-1 sm:flex-initial items-center justify-center whitespace-nowrap rounded-sm px-2.5 py-1 font-medium transition-all text-xs">WEB (<span x-text="webThemeCount"></span>)</button>
                                <button type="button" @click="installedTypeTab = 'admin'" :class="installedTypeTab === 'admin' ? 'bg-background text-foreground shadow-sm' : ''" class="inline-flex flex-1 sm:flex-initial items-center justify-center whitespace-nowrap rounded-sm px-2.5 py-1 font-medium transition-all text-xs">ADMIN (<span x-text="adminThemeCount"></span>)</button>
                            </div>
                        </div>
                    </template>
                </div>
                ...
                <!-- List -->
                <div class="bg-card card-content mt-6 !p-0 border-none overflow-hidden">
                                        ...
                                        <div class="p-4">
                                            <div class="flex items-start justify-between">
                                                    ...
                                                    <div class="flex items-center gap-2 mb-1 flex-wrap">
                                                        ...
                                                        <template x-if="type === 'themes'">
                                                            <span class="text-[10px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded border"
                                                                :class="(item.package_type === 'admin') ? 'border-violet-500 text-violet-600 dark:text-violet-400' : 'border-sky-500 text-sky-600 dark:text-sky-400'"
                                                                x-text="(item.package_type === 'admin') ? 'ADMIN' : 'WEB'"></span>
                                                        </template>
                                                ...
                                                <div class="flex items-center gap-2 ml-4" x-show="type === 'plugins'">
                                                    ...
                                                    <template x-if="loadingItems[item.slug]">
                                                        <div class="inline-flex h-5 w-9 items-center justify-center">
                                                    ...
                                                    <!-- Show toggle button when not loading -->
                                                    <template x-if="!loadingItems[item.slug]">
                                            ...
                                            <div class="flex flex-wrap gap-1.5">
                                                ...
                                                <!-- Action buttons: Active/Disable and Delete -->
                                                <template x-if="type === 'plugins'">
                                                    <div class="flex gap-1.5">
                                                        <!-- Active/Disable button (normal color) -->
                                                        <button 
                                                            @click="toggleItem(item)" 
                                                            class="h-7 px-2 inline-flex items-center justify-center rounded-md text-xs font-medium border border-input bg-background hover:bg-accent hover:text-accent-foreground transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                                                            <i :data-lucide="item.is_active ? 'power' : 'power-off'" class="h-3 w-3 mr-1"></i>
                                                            <span x-text="!!item.is_active ? '<?= __('disable') ?>' : '<?= __('enable') ?>'"></span>
                                                        </button>
                                                        <!-- Delete button (primary color, disabled if active) -->
                                                        <button 
                                                            @click="deleteItem(item)" 
                                                            :disabled="!!item.is_active"
                                                            class="h-7 px-2 inline-flex items-center justify-center rounded-md text-xs font-medium border border-input bg-primary text-primary-foreground hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                                                ...
                                                <template x-if="type === 'themes'">
                                                    <div class="flex flex-col gap-2 w-full">
                                                        <p class="text-[11px] text-muted-foreground" x-show="item.is_active"><?= __('theme switch hint') ?></p>
                                                ...
                                                <!-- Custom buttons from config (Settings, etc.) -->
                                                <template x-if="!!item.is_active && Array.isArray(item.buttons) && item.buttons.length > 0">
                                                    <div class="flex gap-1.5">
                                                        <template x-for="btn in item.buttons" :key="btn.url">
                                                            <a :href="btn.url" class="h-7 px-2 inline-flex items-center justify-center rounded-md text-xs font-medium border border-input bg-background hover:bg-accent hover:text-accent-foreground">
                                                                <i :data-lucide="btn.icon || 'settings'" class="h-3 w-3 mr-1"></i>
                                                                <span x-text="btn.label || '<?= __('settings') ?>'"></span>
                                                            </a>
                                                        </template>
                                                    </div>
                                                </template>
...
function librariesManager(type, items) {
    // Normalize is_active to boolean for all items
    const normalizedItems = Array.isArray(items) ? items.map(item => {
        // Convert is_active to proper boolean
        let isActive = false;
        if (item.is_active === true || item.is_active === 1 || item.is_active === '1' || item.is_active === 'true') {
            isActive = true;
        } else if (item.is_active === false || item.is_active === 0 || item.is_active === '0' || item.is_active === 'false') {
            isActive = false;
        }
        return {
            ...item,
    ...
    return {
        ...
        installedTypeTab: 'all',
        updateCount: 0,
        dragActive: false,
        selectedFiles: [],
        loadingItems: {}, // Track loading state per item
        

        get itemsByTypeTab() {
            if (this.type !== 'themes' || this.installedTypeTab === 'all') return this.items;
            if (this.installedTypeTab === 'admin') return this.items.filter(it => (it.package_type || 'web') === 'admin');
            return this.items.filter(it => (it.package_type || 'web') !== 'admin');
        },
        get filteredByTypeLength() { return this.itemsByTypeTab.length; },
        get webThemeCount() { return this.items.filter(it => (it.package_type || 'web') !== 'admin').length; },
        get adminThemeCount() { return this.items.filter(it => (it.package_type || 'web') === 'admin').length; },
        get filteredInstalledItems() {
            const term = this.installedSearchTerm.toLowerCase();
            let pool = this.itemsByTypeTab;
            let filtered = pool.filter(it =>
        ...
        get activeCountScoped() {
            const pool = this.type === 'themes' ? this.itemsByTypeTab : this.items;
            return pool.filter(p => !!p.is_active).length;
        },
        get inactiveCountScoped() {
            const pool = this.type === 'themes' ? this.itemsByTypeTab : this.items;
            return pool.filter(p => !p.is_active).length;
        },
        ...
        toggleItem(item) {
            ...
            const confirmMessage = this.type === 'themes'
                ? (item.is_active
                    ? '<?= __("Are you sure you want to deactivate this theme?") ?>'
                    : '<?= __("Are you sure you want to activate this theme?") ?>')
                : (item.is_active
                    ? '<?= __("Are you sure you want to deactivate this plugin? This may affect site functionality.") ?>'
                    : '<?= __("Are you sure you want to activate this plugin?") ?>');
            ...
            // Set loading state
            this.loadingItems[item.slug] = true;
            
            fetch('<?= admin_url('libraries/action') ?>', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
                body: JSON.stringify({ action, type: this.type, item: item.slug, [this.type.slice(0, -1)]: item.slug })
            })
            .then(res => res.json())
            .then(data => {
                if (data?.success === true) {
                    item.is_active = !item.is_active;
                    this.showNotification('info', data.message || 'Success');
                    setTimeout(function(){
                        if (confirm('<?= __("Do you want to reload the page to see the changes?") ?>')) {
            ...
            .catch(() => {
                this.showNotification('error', '<?= __("Network error occurred") ?>');
            })
            .finally(() => {
                // Remove loading state
                delete this.loadingItems[item.slug];
            });
        },
        ...
        deleteItem(item) {
            ...
            fetch('<?= admin_url('libraries/action') ?>', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-Requested-With': 'XMLHttpRequest' },
                body: JSON.stringify({ action: 'delete', type: this.type, item: item.slug, [this.type.slice(0, -1)]: item.slug })
            })
            .then(res => res.json())
            .then(data => {
                if (data.success === true) {
                    this.items = this.items.filter(p => p.slug !== item.slug);
                    this.showNotification('info', data.message || 'Deleted');
                } else {
                    this.showNotification('error', data.message || 'Action failed');
                }
            })
        ...
        async uploadAllFiles() {
            ...
            .then(data => {
                if (data.success) { 
                    this.showNotification('success', data.message || '<?= __('upload success') ?>'); 
                    setTimeout(function(){
                        if (confirm('<?= __("Do you want to reload the page to see the changes?") ?>')) {
                            window.location.reload();
                        }
                    ...
                    this.showNotification('error', data.message || '<?= __('upload error') ?>'); 
                }
            })
