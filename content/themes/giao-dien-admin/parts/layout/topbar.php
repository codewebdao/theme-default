<?php
$ui = is_array($user_info ?? null) ? $user_info : [];
$avatar = (string) ($ui['avatar'] ?? '');
$fullname = (string) ($ui['fullname'] ?? '');
$role = (string) ($ui['role'] ?? '');
$email = (string) ($ui['email'] ?? '');
$initial = $fullname !== ''
    ? (function_exists('mb_substr') ? mb_substr($fullname, 0, 1, 'UTF-8') : substr($fullname, 0, 1))
    : '?';
?>
  ...
  <div class="flex items-center space-x-2">
    ...
    <div x-data="{ open: false }" class="relative">
      <button @click="open = !open" class="flex items-center space-x-2 p-0 rounded-md hover:bg-accent">
        <?php if ($avatar !== ''): ?>
          <img src="<?= htmlspecialchars($avatar, ENT_QUOTES, 'UTF-8') ?>" alt="Avatar" class="w-8 h-8 rounded-full">
        <?php else: ?>
          <!-- Name first letter -->
          <div class="w-8 h-8 rounded-full bg-muted flex items-center justify-center">
            <span class="text-sm font-medium text-foreground"><?= htmlspecialchars($initial, ENT_QUOTES, 'UTF-8') ?></span>
          </div>
        <?php endif; ?>
        <div class="hidden lg:flex flex-col items-start">
          <span class="text-sm font-medium text-foreground"><?= htmlspecialchars($fullname, ENT_QUOTES, 'UTF-8') ?></span>
          <span class="text-xs text-muted-foreground"><?= htmlspecialchars($role, ENT_QUOTES, 'UTF-8') ?></span>
        </div>
      ...
      <div x-show="open" @click.away="open = false" x-transition class="absolute right-0 mt-2 w-72 text-popover-foreground border border-border rounded-md shadow-lg z-10 bg-background">
        ...
        <div class="flex items-center space-x-4 p-2 bg-muted/50 rounded-lg">
            <?php if ($avatar !== ''): ?>
              <img src="<?= htmlspecialchars($avatar, ENT_QUOTES, 'UTF-8') ?>" alt="Avatar" class="w-16 h-16 rounded-full">
            <?php else: ?>
              <!-- Name first letter -->
            ...
            <?php endif; ?>
            <div>
                <h3 class="font-medium"><?= htmlspecialchars($fullname, ENT_QUOTES, 'UTF-8') ?></h3>
                <p class="text-sm text-muted-foreground"><?= htmlspecialchars($email, ENT_QUOTES, 'UTF-8') ?></p>
            </div>
        </div>


          <a href="<?= auth_url('profile') ?>" class="flex items-center px-2 py-1.5 text-sm rounded-md hover:bg-accent"><i data-lucide="user-cog" class="mr-2 h-4 w-4"></i> Profile</a>
          <!-- <a href="#"
